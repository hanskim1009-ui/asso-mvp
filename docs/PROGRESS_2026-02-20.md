...
## 9. 의견서 2단계 생성 + 참고자료 RAG

### 9.1 의견서 2단계 생성

- **1차**: 비싼 모델(Claude Opus 4.5 등)로 **목차(outline) + 본문 작성 AI용 지시문(metaPrompt)** 생성.
- **2~4차**: 가성비 모델(Gemini 2.5 Flash 등)로 본문을 파트별로 생성. 사용자가 1차 결과 확인 후 2차·3차·4차를 수동 실행.
- **API**
  - `POST /api/opinion/generate-outline`: Body `analysis`, `userContext`, `opinionType`, `userPrompt`, `model`, `selectedReferenceChunks` → `{ outline, metaPrompt }`.
  - `POST /api/opinion/generate-chunk`: Body `analysis`, `userContext`, `opinionType`, `outline`, `metaPrompt`, `model`, `partIndex`, `previousChunks`, `selectedReferenceChunks` → `{ chunk }`.
- **프론트 (app/cases/[id]/page.js)**
  - 1차/2~3차 AI 모델 드롭다운, 단계별 UI: 설정 → 1차 목차·방향 생성 → 목차/지시문 확인·수정 → 2차 작성 → 필요 시 3차·4차·「처음부터」 리셋.
- **모델**: `lib/opinionPrompts.js`에 Claude Opus 4.5 추가, generate-outline/generate-chunk에서 `MODEL_IDS` 사용.

### 9.2 참고자료 RAG

- 참고자료(대법원 양형기준표, 판례 요약 등) PDF 업로드 → 텍스트 추출·청킹·임베딩·메타데이터 태깅 → 의견서 생성 시 쿼리 기반 벡터 검색으로 상위 K개 청크를 프롬프트에 삽입.
- 설계: `docs/REFERENCE_RAG_DESIGN.md`, 테이블 설정: `docs/REFERENCE_RAG_TABLES_SETUP.md`.
- 의견서 저장·학습 예시: `docs/OPINION_TABLES_SETUP.md`, `docs/OPINION_PERSISTENCE_AND_LEARNING.md`.

### 9.3 빌드 에러 수정 (getOpinionLearningExamples)

- **증상**: "1차 의견서 작성" 실행 시 Build Error — `Export getOpinionLearningExamples doesn't exist in target module` (generate-outline에서 `@/lib/database` import).
- **조치**: `app/api/opinion/generate-outline/route.js`에서 해당 import 제거(학습 예시는 사용하지 않거나 빈 배열). `lib/database.js`에 `getOpinionLearningExamples(opinionType, limit)` export 추가 — `opinion_learning_examples` 테이블 조회, 에러 시 `[]` 반환.
- **참고**: 캐시 이슈 시 dev 서버 중지 후 `.next` 삭제, `npm run dev` 재실행 권장.
