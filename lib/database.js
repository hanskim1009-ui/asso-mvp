...
/**
 * 의견서 종류별 학습 예시 (수정 예시 등). 테이블 없으면 빈 배열 반환.
 */
export async function getOpinionLearningExamples(opinionType, limit = 2) {
  try {
    const { data, error } = await supabase
      .from('opinion_learning_examples')
      .select('*')
      .eq('opinion_type', opinionType ?? 'sentencing')
      .order('created_at', { ascending: false })
      .limit(limit)

    if (error) {
      console.error('getOpinionLearningExamples:', error)
      return []
    }
    return data ?? []
  } catch (err) {
    console.error('getOpinionLearningExamples:', err)
    return []
  }
}
...
export async function updateEvidenceSection(sectionId, updates) {
  try {
    ...
    if (updates.section_memo !== undefined) allowed.section_memo = updates.section_memo
    if (updates.vision_description !== undefined) allowed.vision_description = updates.vision_description
...
/**
 * 참고자료 문서 등록
 * @param {{ title: string, source_description?: string, file_url?: string, file_name?: string }}
 * @returns {Promise<string|null>} reference_document id
 */
export async function insertReferenceDocument(data) {
  try {
    ...
    console.error('insertReferenceDocument:', err)
    throw err
  }
}
...
export async function getReferenceDocument(id) {
  try {
    ...
    console.error('getReferenceDocument:', err)
    return null
  }
}
 ...
 * 참고자료 문서 목록 (최신순)
 */
export async function getReferenceDocuments(limit = 50) {
  try {
    const { data, error } = await supabase
      .from('reference_documents')
      .select('*')
    ...
    console.error('getReferenceDocuments:', err)
    return []
  }
}

/**
 * 참고자료 문서 삭제 (cascade로 청크도 삭제)
 */
export async function deleteReferenceDocument(id) {
  try {
    ...
    console.error('deleteReferenceDocument:', err)
    throw err
  }
}
