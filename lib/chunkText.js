 ...
 * 제○장, 제○조, ○. ○○죄, 가. 나. 다., ① ②, (가)(나) 등으로 구간 나눈 뒤, 구간별로 chunkSize 이하로 유지
 * @param {string} text
 * @param {number} chunkSize
 * @returns {Array<{ text: string, pageNumber: number, chunk_index: number }>}
 */
export function chunkBySections(text, chunkSize = REFERENCE_DEFAULT_CHUNK_SIZE) {
  ...
  const sections = []
  let current = []
  // 법률 문서 흔한 패턴: 제1장, 제2절, 제1조, 1. 절도죄, 가. 나. 다., ① ② ③, (가) (나), 1) 2)
  const sectionStartRe = /^(제\s*\d+\s*[장절조항]|\d+\.\s+.+|[가나다라마바사아자차카타파하]\s*\.\s*|[\u2460-\u2473\u3251-\u325F\u32B1-\u32BF]|\d+\)\s*|\(\s*[가나다라마바사아자차카타파하]\s*\))\s*/
