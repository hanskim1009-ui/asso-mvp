...
/**
 * Hybrid 검색: 벡터 + 전문 검색(query_text) RRF 병합.
 * match_reference_chunks_hybrid RPC가 없거나 content_tsv가 없으면 벡터 검색만 사용.
 * @param {string} queryText
 * @param {number} matchCount
 * @returns {Promise<Array>}
 */
export async function searchReferenceChunksHybrid(queryText, matchCount = 5) {
  if (!queryText?.trim()) return []
  const queryTrimmed = queryText.trim()
  const embedding = await getEmbedding(queryTrimmed)
  try {
    const { data, error } = await supabase.rpc('match_reference_chunks_hybrid', {
      query_embedding: embedding,
      query_text: queryTrimmed,
      match_count: matchCount,
    })
    if (error) throw error
    return data ?? []
  } catch (err) {
    console.warn('match_reference_chunks_hybrid fallback to vector-only:', err?.message)
    return searchReferenceChunks(embedding, matchCount)
  }
}
